...
var insertQuoteRequestSchema = createInsertSchema(quoteRequests).omit({
  ...
  company: z.string().min(1, "Nome da empresa \xE9 obrigat\xF3rio"),
  service: z.string().min(1, "Servi\xE7o \xE9 obrigat\xF3rio"),
...
var DatabaseStorage = class {
  ...
  async createQuoteRequest(insertRequest) {
    const [request] = await db.insert(quoteRequests).values(insertRequest).returning();
    return request;
  }
  async getQuoteRequests() {
    const results = await db.select().from(quoteRequests).orderBy(desc(quoteRequests.createdAt));
    return results;
  }
  ...
  app2.post("/api/quote-requests", async (req, res) => {
    try {
      ...
      res.status(201).json({
        success: true,
        data: quoteRequest,
        message: "Solicita\xE7\xE3o de or\xE7amento enviada com sucesso!"
      });
    } catch (error) {
